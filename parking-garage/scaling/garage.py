...
class Parking:
    def __init__(self):
        ...
        # self.times = [np.zeros(spaces_per_type[indx], dtype=np.int) for inx in range(nspace_types)]
        # Array of car types per parking space (needed for cost calculation)
        self.cars = [[None for _ in range(spaces_per_type[indx])]
                      for indx in range(nspace_types)]
    ...
    def alloc_handi_sp(self, time, large_car):
        try:
            ...
            try:
                # slot_small = self.times[indx_small].index(default_time)
                slot_small = self.get_slot(i_set)
            except:
                slot_small = err # no small slots available
            try:
                # slot_large = self.times[indx_large].index(default_time)
                slot_large = self.get_slot(i_set)
                if slot_small == err:
                    return self.alloc_open_sp(time, large_car, indx_handi)
            except:
                if slot_small == err:
    ...
    def alloc(self, time, i_set, i_type):
        try:
            # i_slot = self.times[i_set].index(default_time)
            i_slot = self.get_slot(i_set)
    ...
    def get_slot(self, i_set):
        if skip_lookup:
            return 1 # skip array lookup for profiling timing
        time_arr = self.times[i_set]
        return time_arr.index(default_time)
        # Numba JIT is really slow processing Python list:
        #    find_Slot(default_time, time_arr)
""" @jit(nopython=True)
